...
int main(int argc, char *argv[])
{
  ...
  while( 1 ) {
    ...
    default:
      ...
      cerr << "Usage: glite-ce-monitor-pause [--cert <CERTIFICATE_FILE>] [--key <KEY_FILE>] --id <SUBSCRIPTION_ID_TO_REMOVE> <CEMon_URL>"<< endl;
      exit(1);
    } // switch
  }  // while
  ...
  if(argv[optind]==NULL) {
    ...
    cerr << "The subscription ID is mandatory... " << endl;
    cerr << "Usage: glite-ce-monitor-pause [--cert <CERTIFICATE_FILE] [--key <KEY_FILE>] --id <SUBSCRIPTION_ID_TO_PAUSE> <CEMon_URL>"<< endl;
    exit(1);
